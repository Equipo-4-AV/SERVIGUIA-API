...
export function ExamplePrompts({ onPick }: { onPick: (text: string) => void }) {
  return (
    <div className="rounded-2xl border border-dashed border-border bg-[image:var(--gradient-subtle)] p-6">
        ...
        {EXAMPLES.map((ex) => {
          ...
          return (
            <button
              ...
              className="group rounded-xl border border-border bg-card px-4 py-3 text-left transition-all hover:border-primary/40 hover:bg-primary/5 hover:shadow-[var(--shadow-card)]"
            >
              <div className={`mb-1 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider ${toneStyles[ex.tone]}`}>
                <Icon className="h-3 w-3" />
                {ex.label}
              </div>
              <p className="text-sm text-card-foreground">{ex.text}</p>
            </button>
          );
        })}
      </div>
